...
# diag(Sigma) = 1
# Sigma = AR1(p, rho)
Noise = rmvn.sparse(n, mu = rep(0, p), CH=Cholesky(Sigma))
fctr = mean(apply(eta,1, var)) * (1-sigFrac)/sigFrac
Y = eta + Noise*fctr  

modelSearch = bestSubsetSearch(Y, as.data.frame(X), colnames(X), maxk=1)
...
# mvIC(fit, shrink.method="EB")
# mvIC(fit, shrink.method="var_unequal")
```

```{r test methods}
set.seed(2)
res = lapply( c(seq(10, 90, by=10), seq(100, 1000, by=200)), function(p){
  n = 50
  m_active = 1
  m_total = 10
  rho = .8
  ...
  fit = lm(as.formula(trueModel), data=as.data.frame(X))
  lambda = mvIC(fit, shrink.method="EB")@params$lambda

  # b = beam::beam(t(mvIC:::getResids(fit)) )
  # b@alphaOpt
  # b@valOpt

  data.frame(lambda, p, rank=df[isTrue==TRUE,1:3][2,3], Strimmer = attr(cov.shrink(t(mvIC:::getResids(fit)), lambda.var=0, verbose=FALSE), "lambda"), Touloumis = shrinkcovmat.unequal((mvIC:::getResids(fit)))$lambdahat, eb_cov_est3 = mvIC:::eb_cov_est3(t(mvIC:::getResids(fit)))$alpha, gcShrink = gcShrink((mvIC:::getResids(fit)), plot=FALSE)$optimalpha )
})
...
# mvIC:::eb_cov_est(mvIC:::getResids(fit))

# str(mvIC:::eb_cov_est2(mvIC:::getResids(fit)))


# str(mvIC:::eb_cov_est3(t(mvIC:::getResids(fit))))
