...
m_active_array = 1:2#c(1, 2, 3)
signalFraction = .seq(.2, .8, by=.2)
ngenes_array = c(10, 50, 100, 200) #c(round(seq(10, 100, length.out=5), 0), 200, 500, 1000)
    ...
    resRecovery = lapply(ngenes_array, function(p){
        ...
        resRecovery = lapply( rho_array, function(rho){
          ...
          resRecovery = lapply(signalFraction, function(sigFrac){
            ...
            methods = c( "var_equal", "var_unequal") 
...
resRecovery = lapply( rho_array, function(rho){
  resRecovery = lapply(1:n_reps, function(k){
    ...
    methods = c( "var_unequal")#, "var_equal")  #"Strimmer" , "pseudodet"
    ...
    # fit naive model
    bestModelNaive = mvForwardStepwise( t(Y), ~1, X, colnames(X), usemvIC=FALSE, verbose=FALSE, deltaCutoff=5)
    ...
    data.frame( method = c(paste("mvIC", methods, sep=' - '), 'naive'),
...
# naive
mvIC_fit( t(Y), ~ 1, X, usemvIC=FALSE)
mvIC_fit( t(Y), ~ X_1, X, usemvIC=FALSE)

method = "var_unequal"
# method="var_equal"
res1 = mvIC_fit( t(scale(Y)), ~ X_1 , X, shrink.method=method)
res1@params
res2 = mvIC_fit( t(scale(Y)), ~ 1, X, method)
res2@params
res1-res2
...
# sum(log(adjusted_eigen_values(residMatrix1, method="var_unequal")))
# sum(log(adjusted_eigen_values(residMatrix2, method="var_unequal")))

# sum(log(adjusted_eigen_values(residMatrix1, method="var_equal")))
# sum(log(adjusted_eigen_values(residMatrix2, method="var_equal")))

res1 = rlogDet( residMatrix1, method="var_unequal" )
res2 = rlogDet( residMatrix2, method="var_unequal" )
res1 - res2

resA = rlogDet( residMatrix1, method="var_equal" )
resB = rlogDet( residMatrix2, method="var_equal" )
resA - resB
