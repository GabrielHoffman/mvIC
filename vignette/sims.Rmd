...
signalFraction = #seq(.2, .8, by=.2)
ngenes_array = c(10, 50, 200)#c(round(seq(10, 100, length.out=5), 0), 200, 500)
rho_array = seq(0, .98, length.out=6)
...
resRecovery = lapply(1:n_reps, function(k){
  ...
  resRecovery = lapply(m_active_array, function(m_active){
    ...
    resRecovery = lapply(ngenes_array, function(p){
        ...
        clustID = sample.int(n_clusters, p, replace=TRUE)
        ...
        resRecovery = lapply( rho_array, function(rho){
          ...
          resRecovery = lapply(signalFraction, function(sigFrac){
            ...
            methods = c( "Touloumis_unequal", "Touloumis_equal")
            criteria = c("AIC", "AICC", "BIC") 
            res = lapply( methods, function(method){
              res = lapply( criteria, function(criterion){
                bestModel = mvForwardStepwise( t(Y), ~1, X, colnames(X), criterion=criterion, verbose=FALSE, logDetMethod=method, deltaCutoff=5)
            ...
            # fit naive model
            bestModelNaive = mvForwardStepwise( t(Y), ~1, X, colnames(X), criterion="sum BIC", verbose=FALSE, deltaCutoff=5)
            ...
            df_add = data.frame( criterion = "BIC", 
                        method = "sum BIC",
...
cols = c("red", "orange", "dodgerblue", "navy", "grey", "green", "black")
...
# pdf("~/www/mvBIC.pdf", width=12, height=120)
...
resRecovery = lapply( rho_array, function(rho){
  resRecovery = lapply(1:n_reps, function(k){
    ...
    # try 3 logDet methods
    methods = c( "Touloumis_unequal")#, "Touloumis_equal")  #"Strimmer" , "pseudodet"
...
figList = lapply(unique(df$sigFrac), function(v){
ggplot(subset(df, sigFrac==v), aes(rho, recoveryRate, color=method, fill=method)) + geom_ribbon(aes(ymin=down, ymax=up), alpha=.3, linetype=0) + geom_line() + geom_point() + scale_color_manual("Method", values = cols ) + scale_fill_manual("Method", values = cols ) + xlab(bquote(Correlation~(rho))) + ylab("Power to recover true model") + theme_bw() + theme(aspect.ratio=1) + ylim(0, 1) + facet_wrap(~m_active+p, ncol=length(ngenes_array)  ) + ggtitle(v)
})
...
# method="Touloumis_equal"
res1 = mvBIC_fit( t(scale(Y)), ~ X_1 , X, logDetMethod=method)
res1@params
res2 = mvBIC_fit( t(scale(Y)), ~ 1, X, logDetMethod=method)
res2@params
res1-res2
...
# sum(log(adjusted_eigen_values(residMatrix1, method="Touloumis_unequal")))
# sum(log(adjusted_eigen_values(residMatrix2, method="Touloumis_unequal")))

# sum(log(adjusted_eigen_values(residMatrix1, method="Touloumis_equal")))
# sum(log(adjusted_eigen_values(residMatrix2, method="Touloumis_equal")))

res1 = rlogDet( residMatrix1, method="Touloumis_unequal" )
res2 = rlogDet( residMatrix2, method="Touloumis_unequal" )
res1 - res2

resA = rlogDet( residMatrix1, method="Touloumis_equal" )
resB = rlogDet( residMatrix2, method="Touloumis_equal" )
resA - resB




mvBIC:::mvBIC_from_residuals( residMatrix1, 3, logDetMethod="Touloumis_unequal")
mvBIC:::mvBIC_from_residuals( residMatrix2, 2, logDetMethod="Touloumis_unequal"  )
