...
set.seed(1)
res = lapply( c(seq(10, 90, by=10), seq(100, 1000, by=50)), function(p){
  n = 50
  m_active = 1
  m_total = 10
  rho = 0
  ...
  data.frame(lambda, p, rank=df[isTrue==TRUE,1:3][2,3])#, Strimmer = attr(cov.shrink(t(mvIC:::getResids(fit)), lambda.var=0, verbose=FALSE), "lambda"), Touloumis = shrinkcovmat.unequal((mvIC:::getResids(fit)))$lambdahat, test = test_run(t(mvIC:::getResids(fit))))
# gcShrink = gcShrink((mvIC:::getResids(fit)))$optimalpha )
})
