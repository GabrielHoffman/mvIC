...
signalFraction = seq(.01, .99, length.out=5)
  ...
  resRecovery = lapply(signalFraction, function(sigFrac){
    ...
    resRecovery = lapply(ngenes_array, function(p){
      ...
      resRecovery = lapply( seq(0, .95, length.out=5), function(rho){
          ...
          methods = c( "Touloumis_equal", "pseudodet")  #"Strimmer" "Touloumis_unequal"
...
# summarize 
df = resRecovery[,data.frame(recoveryRate = sum(recovery)/length(recovery)),by=c('rho', 'k', 'p', 'sigFrac', 'm_active','method')]
