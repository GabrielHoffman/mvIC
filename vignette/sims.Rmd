...
m_active = 1 # number of variables affecting phenotype
m_total = 50 # total number of variables

 # summary(lm(Y~X[,1]))
# lapply(coef(summary(lm(Y~X[,1]))), function(fit) fit[2,4])

m_active_array = c(1, 2, 3)#, 4, 5)
signalFraction = 1 #seq(.01, .99, length.out=5)
ngenes_array = c(10, 20, 100, 300)#500, 1000, 2000, 500, 10000)
...
resRecovery = lapply(1:n_reps, function(k){
  ...
  resRecovery = lapply(signalFraction, function(sigFrac){
    ...
    # simulate variables
    X = matrix(rnorm(m_total*n), n, m_active)
    colnames(X) = paste0('X_', 1:m_active)
    ...
    resRecovery = lapply(ngenes_array, function(p){
      ...
      resRecovery = lapply( seq(0, .95, length.out=5), function(rho){
          ...
          # sigFrac = .3
          # fctr = var(eta[1,])* (1-sigFrac)/sigFrac

          # Y = eta + Noise*fctr
          # cor(eta[1,], eta[1,] + Noise[1,]*fctr)^2

          message("\rk = ", k, '  rho = ', rho, '    ')   
...
# summarize 
df = resRecovery[,data.frame(recoveryRate = sum(recovery)/length(recovery)),by=c('rho', 'k', 'p', 'm_active','method')]
