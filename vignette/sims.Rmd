...
res = mvIC:::estimate_covariance( t(mvIC:::getResids(fit)))
lambda = res$lambda

# R = t(mvIC:::getResids(fit))
# CovEst.2003LW(R, diag(apply(R, 2, var))) 


modelSearch = bestSubsetSearch(Y, as.data.frame(X), colnames(X), maxk=2, lambda=lambda)

modelSearch = data.table(modelSearch)
df = modelSearch[,data.frame(rank = rank(score), isTrue = form==trueModel) ,by=c('method', 'criterion')]
df[isTrue==TRUE,1:3]

modelSearch[method=="EB" & criterion=="BIC",]


fit = lm(Y ~ 1, data=as.data.frame(X))
res = estimate_covariance( t(mvIC:::getResids(fit)))
res


fit = lm(as.formula(trueModel), data=as.data.frame(X))
mvIC:::f_penalized_logLik( lambda, n, p, t(mvIC:::getResids(fit)))
mvIC(fit, criterion = "BIC", shrink.method="EB", lambda=lambda)

fit1 = lm(Y ~ X_1 + X_2, data=as.data.frame(X))
mvIC:::f_penalized_logLik( lambda, n, p, t(mvIC:::getResids(fit1)))
mvIC(fit1, criterion = "BIC", shrink.method="EB", lambda=lambda)


fit = lm(as.formula(trueModel), data=as.data.frame(X))
estimate_covariance( t(mvIC:::getResids(fit)))


fit = lm(Y ~ X_2, data=as.data.frame(X))
estimate_covariance( t(mvIC:::getResids(fit)))
...
res = lapply( c(seq(10, 90, by=10), seq(100, 1100, by=50)), function(p){
  ...
  tst1 = mvIC:::estimateMVN_EB( (t(residMatrix)), MAP=TRUE )
  # tst2 = gcShrink(  t((t(residMatrix))), var=3, cor=1, plot=FALSE)
  tst2 = mvIC:::eb_cov_est( (residMatrix))
  tst3 = shrinkcovmat.unequal( residMatrix )
...
ngenes_array = c(10, 20, 30, 40, 60, 80, 100, 200, 1000)
    ...
    resSearch = lapply(ngenes_array, function(p){
        ...
        resSearch = lapply( rho_array, function(rho){
          ...
          resSearch = lapply(signalFraction, function(sigFrac){
            ...
            modelSearch = bestSubsetSearch(Y, as.data.frame(X), colnames(X), maxk=3, lambda=lambda)
            modelSearch = data.table(modelSearch)
...
```{r plot.model.search, cache=FALSE, fig.width=20, fig.height=10}
